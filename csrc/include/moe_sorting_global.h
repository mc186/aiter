...
struct MoeSortingGlobalHostArgs
{
...
struct MoeSortingGlobalKernel
{
    ...
    typedef MoeSortingGlobalHostArgs MoeSortingGlobalKargs;

    using Hargs = MoeSortingGlobalHostArgs;
    ...
    CK_TILE_HOST static constexpr auto MakeKargs(const Hargs& h)
    {
        ...
        k.p_tokens_cnts            = h.p_tokens_cnts;
        k.p_cumsum                = h.p_cumsum;
    ...
    CK_TILE_DEVICE void moe_align_block_size_kernel(const IndexType* __restrict__ topk_id,
                                                    ...
                                                    index_t* p_tokens_cnts,
                                                    index_t* p_cumsum,
                                                    const index_t num_experts,
                                                    const index_t tokens_per_thread,
                                                    const index_t numel,
                                                    const mdiv unit_size_mdiv,
                                                    const mdiv topk_mdiv) const
    {
        const index_t tid       = static_cast<index_t>(threadIdx.x);
        const index_t start_idx = tid * tokens_per_thread;

        // index_t* p_tokens_cnts = shared_mem; // 2d: (blockDim.x + 1, num_experts)
        // index_t* p_cumsum      = shared_mem + (blockDim.x + 1) * (num_experts+1); // 1: (num_experts + 1)

        for(int i = 0; i < num_experts; ++i)
        {
            p_tokens_cnts[calc_index(num_experts+1, tid + 1, i)] = 0;
        }

#pragma unroll Problem_::InternalLoadUnroll
        for(int i = start_idx; i < numel && i < start_idx + tokens_per_thread; ++i)
        {
            ++p_tokens_cnts[calc_index(num_experts+1, tid + 1, topk_id[i])];
        }
        __syncthreads();
        ...
        if(tid < num_experts)
        {
            p_tokens_cnts[calc_index(num_experts+1, 0, tid)] = 0;
            index_t local_c[8];
            index_t prev_c = 0;
            ...
            {
                local_c[0] = p_tokens_cnts[calc_index(num_experts+1, i + 0, tid)];
                local_c[1] = p_tokens_cnts[calc_index(num_experts+1, i + 1, tid)];
                local_c[2] = p_tokens_cnts[calc_index(num_experts+1, i + 2, tid)];
                local_c[3] = p_tokens_cnts[calc_index(num_experts+1, i + 3, tid)];
                local_c[4] = p_tokens_cnts[calc_index(num_experts+1, i + 4, tid)];
                local_c[5] = p_tokens_cnts[calc_index(num_experts+1, i + 5, tid)];
                local_c[6] = p_tokens_cnts[calc_index(num_experts+1, i + 6, tid)];
                local_c[7] = p_tokens_cnts[calc_index(num_experts+1, i + 7, tid)];
                ...
                p_tokens_cnts[calc_index(num_experts+1, i + 0, tid)] = local_c[0];
                p_tokens_cnts[calc_index(num_experts+1, i + 1, tid)] = local_c[1];
                p_tokens_cnts[calc_index(num_experts+1, i + 2, tid)] = local_c[2];
                p_tokens_cnts[calc_index(num_experts+1, i + 3, tid)] = local_c[3];
                p_tokens_cnts[calc_index(num_experts+1, i + 4, tid)] = local_c[4];
                p_tokens_cnts[calc_index(num_experts+1, i + 5, tid)] = local_c[5];
                p_tokens_cnts[calc_index(num_experts+1, i + 6, tid)] = local_c[6];
                p_tokens_cnts[calc_index(num_experts+1, i + 7, tid)] = local_c[7];
            }
        }
#else
        // TODO: below code still working, but slow in expert=32/topk=5 case. Put here for future heuristic
        {
            ...
            for(int i = 0; i < num_experts; i+=8) {
                ...
                #pragma unroll
                for(int j = 0; j < 8; j++) {
                    local_c[j] = p_tokens_cnts[calc_index(num_experts+1, tid+1, i+j)];
                }
                ...
                #pragma unroll
                for(int j = 0; j < 8; j++) {
                    p_tokens_cnts[calc_index(num_experts+1, tid+1, i+j)] = local_c[j];
                }
            }
        }
#endif
        ...
        if constexpr (Problem::ExpertTile == 0) {
            if(tid == 0)
            {
                ...
                {
                    auto current_units = [&]() {
                        index_t x_ = p_tokens_cnts[calc_index(num_experts+1, blockDim.x, i - 1)] +
                                    unit_size_mdiv.divisor - 1;
                        index_t y_ = unit_size_mdiv.div(x_);
                        return max(y_, 1) * unit_size_mdiv.divisor;
                    }();
                    p_cumsum[i] = p_cumsum[i - 1] + current_units;
                }
                *p_total_tokens_post_pad = p_cumsum[num_experts];
            }
        } else {
            // TODO: we have out-of-bound read here. But result is still OK (will ignore tid >= expert)
            // for simplicity, not check experts here.
            int local_cnt = p_tokens_cnts[calc_index(num_experts+1, blockDim.x, tid)];
        ...
        if(tid < num_experts)
        {
            int e_start = p_cumsum[tid];
            int e_end = p_cumsum[tid + 1];
...
#pragma unroll Problem_::InternalLoadUnroll
        for(int i = start_idx; i < numel && i < start_idx + tokens_per_thread; ++i)
        {
            ...
            index_t local_cnt = p_tokens_cnts[calc_index(num_experts+1, tid, expert_id)];
            index_t rank_post_pad = local_cnt + p_cumsum[expert_id];
            ...
            p_tokens_cnts[calc_index(num_experts+1, tid, expert_id)] = local_cnt+1;
        }
        ...
        if constexpr (Problem::ExpertTile == 0) {
            ...
            if(tid < num_experts)
            {
                index_t expert_offset =
                    p_cumsum[tid] + p_tokens_cnts[calc_index(num_experts+1, blockDim.x, tid)];
                index_t expert_end = p_cumsum[tid + 1];
            ...
            static constexpr index_t experts_per_wave = warpSize / Problem::ExpertTile;
            {
                ...
                index_t expert_offset =
                    p_cumsum[eid] + p_tokens_cnts[calc_index(num_experts+1, blockDim.x, eid)] + tid % experts_per_wave;
                index_t expert_end = p_cumsum[eid + 1];
                if(eid < num_experts) {
                    while(expert_offset < expert_end)
                    {
#if CK_TILE_REFERENCE_MOE_SORTING_MOCK_ID
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        return moe_align_block_size_kernel(static_cast<const IndexType*>(kargs.p_topk_ids),
                                           ...
                                           static_cast<IndexType*>(kargs.p_tokens_cnts),
                                           static_cast<IndexType*>(kargs.p_cumsum),
                                           kargs.num_experts,
...
struct moe_sorting_global_trait
{
    std::string index_type;
    std::string weight_type; // currently always float
};

struct moe_sorting_global_args : public ck_tile::MoeSortingGlobalHostArgs
{
};

float moe_sorting_global(moe_sorting_global_trait t, moe_sorting_global_args a, ck_tile::stream_config s);
