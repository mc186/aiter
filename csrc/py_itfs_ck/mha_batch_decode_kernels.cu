...
mha_batch_decode(at::Tensor &q,                // [b, sq, hq, d]
    ...
    auto softmax_lse_accum = torch::empty({batch_size, num_heads, num_splits, seqlen_q}, opts.dtype(at::kFloat));
    auto out_accum = torch::empty({batch_size, num_heads, num_splits, seqlen_q, head_size_v}, opts.dtype(at::kFloat));

    auto stream = at::cuda::getCurrentHIPStream().stream();
    ck_tile::stream_config stream_config{stream};

    auto traits =
        get_ck_fmha_batch_decode_traits(
            q_dtype_str,
            head_size_q,
            head_size_v,
            has_logits_soft_cap,
