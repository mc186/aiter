...
#define MOE_SORTING_DISPATCH_ETILE(unroll_num_, expert_tile_)                         \
    ...
    using kernel         = ck_tile::MoeSortingGlobalKernel<ms_problem>;               \
...
float moe_sorting_global(moe_sorting_global_trait t, moe_sorting_global_args a, ck_tile::stream_config s)
{
    if(t.weight_type == "fp32" && t.index_type == "int32")
    {
        ...
        index_t gmem_io_unroll_num = ck_tile::integer_divide_ceil(a.tokens * a.topk, 64);
        switch(gmem_io_unroll_num)
        {
        case(1): {
