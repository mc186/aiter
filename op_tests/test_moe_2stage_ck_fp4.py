...
def test_fmoe(
    ...
    if qType == aiter.QuantType.per_Tensor:
        ...
        w1_qt, w1_scale = torch_quant(
            w1.view(-1, model_dim), quant_dtype=WQDType, shuffle=False
        )
        w2_qt, w2_scale = torch_quant(
            w2.view(-1, inter_dim), quant_dtype=WQDType, shuffle=False
        )
    else:
    ...
    if qType == aiter.QuantType.per_1x32:
        ...
        w1_qt_aiter, w1_scale_aiter = torch_quant(
            w1.view(-1, model_dim), quant_dtype=WQDType, shuffle=True
        )
        w2_qt_aiter, w2_scale_aiter = torch_quant(
            w2.view(-1, inter_dim), quant_dtype=WQDType, shuffle=True
        )
